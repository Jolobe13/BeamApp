...
class Calculadora():
    ...
    def graficas(self):
        while True:
            ...
            valores = {}
            print(todos_los_simbolos) #{L2, x, L1, q}
            for s in todos_los_simbolos:
                if s != self.x and s not in self.L :
                    valor = float(input(f"Ingrese el valor numérico para {s}: "))
                    valores[s] = valor
            #print(valores) {L2: 1.0, L1: 1.0, q: 1.0}

            # Dominio
            L_val = {}
            if 'L' in [str(s) for s in todos_los_simbolos]:
                L_val[f"L"] = float(input(f"Ingrese el valor numérico para L: "))
                for s in range(0, int(self.n)):
                    L_val[f"L{s+1}"]=L_val[f"L"]
            else:

                for s in range(0,int(self.n)):
                    L_val[f"L{s+1}"]=float(input(f"Ingrese el valor numérico para {self.L[s]}: "))
            ...
            for i, expr in enumerate(expresiones):
                ...
                expr_num=expr_num.subs(L_val)
                print(expr_num.free_symbols)
                x_local = np.linspace(0, L_val[f"L{i+1}"], 200)
                ...
                plt.plot(x_global, y_vals, label=f"Ecuación Tramo{i + 1}")
                x_offset=x_offset+L_val[f"L{i+1}"]
