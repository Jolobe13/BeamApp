...
from canvasfile import Dibujar
import math
import sympy as sp
from calculosmatlab2 import Calculadora
from sympy import symbols, Symbol, simplify, integrate, diff, sin,solve,sympify, tan, cos, pretty,lambdify,pi
import numpy as np
...
class VigaSolverApp:
    def __init__(self, root):
        ...
        self.root.geometry("1800x800")  # Aumentamos el tamaño de la ventana
        ...
        ttk.Label(self.left, text="BEAMApp", font=font_label).pack(side=TOP, anchor="w", pady=5)

        # Derecha: Canvas dibujo con scroll
        self.right_top = ttk.Frame(self.main_frame, padding=5, borderwidth=3, relief="solid")
        self.right_top.pack()

        # Label encima del canvas
        self.canvas_label = ttk.Label(self.right_top, text="Dibujo de la viga", font=("Segoe UI", 12, "bold"))
        self.canvas_label.pack(pady=(0, 5))

        # Frame para canvas + scrollbars
        canvas_frame = ttk.Frame(self.right_top)
        canvas_frame.pack()
        ...
        # Canvas
        #self.canvas = tk.Canvas(canvas_frame, bg="white", width=500, height=400,xscrollcommand=scrollbar_x.set,yscrollcommand=scrollbar_y.set)
        self.canvas = tk.Canvas(canvas_frame, bg="white", width=500, height=400)
        self.canvas.pack(side="left", fill="both", expand=True)

        # Vincular scrollbars al canvas
        #scrollbar_x.config(command=self.canvas.xview)
        #scrollbar_y.config(command=self.canvas.yview)

        # Región desplazable (puedes ajustarla según tu dibujo)
        ...
        self.zoom_frame = ttk.Frame(self.main_frame, padding=5)
        self.zoom_frame.pack()

        zoom_factor = 1.1  # Puedes ajustar este factor
        # Factor de zoom
        #zoom_factor = 1.1
        ...
        def zoom(event):
            if event.delta > 0:  # Zoom in
                ...
                self.canvas.scale("all", event.x, event.y, 1 / zoom_factor, 1 / zoom_factor)

            # Ajustamos scrollregion para que el canvas se actualice correctamente
            self.canvas.configure(scrollregion=self.canvas.bbox("all"))

        self.canvas.bind("<MouseWheel>", zoom)  # En Windows
        ...
        def zoom_out():
            self.canvas.scale("all", 0, 0, 1 / zoom_factor, 1 / zoom_factor)
            self.canvas.configure(scrollregion=self.canvas.bbox("all"))

        # === ENTRE CANVAS Y GRÁFICA: BOTONES DE ZOOM ===
        self.zoom_in_button = ttk.Button(self.zoom_frame, text="Zoom +", command=zoom_in)
        self.zoom_in_button.pack(side=LEFT, padx=10)

        self.zoom_out_button = ttk.Button(self.zoom_frame, text="Zoom -", command=zoom_out)
        self.zoom_out_button.pack(side=LEFT, padx=10)

        # Derecha: Texto scrolleable con soluciones
        self.right_bottom = ttk.Frame(self.main_frame, padding=20)
        self.right_bottom.pack(side=BOTTOM, fill=BOTH, expand=True)

        self.figure_graficas, self.ax_graficas = plt.subplots(figsize=(7, 5))  # Gráfico más grande
        self.chart = FigureCanvasTkAgg(self.figure_graficas, self.right_bottom)
        self.chart.get_tk_widget().pack(fill=BOTH, expand=True)

        ttk.Label(self.left, text="SOLUCIONES", font=("Segoe UI", 14, "bold")).pack()
        self.solution_text = ScrolledText(self.left, wrap=tk.WORD, height=25, font=("Courier New", 12))
        ...
        self.n_entry.pack(side=LEFT,fill=X, padx=5,pady=5,expand=True)
        self.n_entry.bind("<Return>", self.on_responder)#permitimos que con darle al enter se evalue la respuesta

        self.boton_reiniciar = tk.Button(self.left_bottom, text="Reiniciar", command=self.reiniciar, width=20)
        self.boton_reiniciar.pack(side=RIGHT, fill=BOTH, padx=5, pady=5)
        self.boton_responder=tk.Button(self.left_bottom, text="Responder", command=self.on_responder, width=20)
        self.boton_responder.pack(side=RIGHT, fill=BOTH,padx=5,pady=5)
        #Calculadora().pedir_input("Indique el número de tramos: ",self.solution_text)
        #self.dibujar_viga([1,2])
    def crear_variables(self):
        self.preguntas = ["Introduzca el tipo de problema (portico/barras simples): ","Introduzca el número de tramos: ", "Desea longitudes iguales? (s/n): "]
        self.incognita = ""
        ...
        if self.pregunta_actual == self.preguntas[0]:
            ...
            if self.respuesta_tipo_de_problema == "portico":
                for i in range(0, int(self.respuestas_tramos[0])):
                    self.tipos_angulos.append(f"Introduzca el angulo en grados (0/90/270) de alpha{i + 1}:")
            elif self.respuesta_tipo_de_problema == "barras simples":
                self.tipos_angulos.append(f"Introduzca el angulo en grados [0;360] de alpha1: ")
            self.pregunta_actual = self.tipos_angulos[self.indice_angulos]
            ...
            if self.indice_angulos < len(self.tipos_angulos):
                ...
                if self.respuesta_tipo_de_problema=="portico":
                    ...
                    try:
                        ...
                        pass
                Dibujar(self.canvas,self.respuestas_tramos[0],self.respuesta_angulos,self.respuesta_tipo_de_problema,"n")
                ...
                for i in range(0,int(self.respuestas_tramos[0])+1):
                    self.tipos_apoyos.append(f"Apoyo {i+1}:")
                self.indice_apoyos=0
                self.pregunta_actual=self.tipos_apoyos[self.indice_apoyos]
                self.respuesta=""

            print(self.tipos_angulos)
            print(self.respuesta_angulos)
                ...
                if self.hay_tramos_moviles=="si":
                    ...
                    for i in range(0,self.indice_apoyos):
                        if self.respuestas_apoyos[i]=="2":
                            self.posicion_apoyos.append(f"Ingrese la posicion del apoyo movil {i+1} (vertical/horizontal):")
                    self.pregunta_actual=self.posicion_apoyos[self.indice_tramos_moviles]
                else:
                    for i in range(1, int(self.respuestas_tramos[0])+1):
                        self.tipos_cargas.append(f"Ingrese componente cortante de la carga del tramo {i}:")
                        self.tipos_cargas.append(f"Ingrese componente axial de la carga del tramo {i}:")
                        self.simbolos_conocidos[f"q{i}"] = Symbol(f"q{i}")
                        self.simbolos_conocidos[f"Q{i}"] = Symbol(f"Q{i}")
                        self.simbolos_conocidos[f"L{i}"] = Symbol(f"L{i}")
                        self.simbolos_conocidos[f"alpha{i}"] = Symbol(f"alpha{i}")
                        self.simbolos_conocidos[f"beta{i}"] = Symbol(f"beta{i}")
            ...
            if self.indice_tramos_moviles < len(self.posicion_apoyos):
                ...
                for i in range(1, int(self.respuestas_tramos[0]) + 1):
                    ...
                    self.simbolos_conocidos[f"alpha{i}"] = Symbol(f"alpha{i}")
                    self.simbolos_conocidos[f"beta{i}"] = Symbol(f"beta{i}")
                for i in range(int(self.respuestas_tramos[0]) + 1, int(self.respuestas_tramos[0]) + int(self.respuestas_tramos[0])+1):
                    self.simbolos_conocidos[f"Q{i}"] = Symbol(f"Q{i}")
                self.indice_cargas = 0
                self.pregunta_actual = self.tipos_cargas[self.indice_cargas]
            ...
            if self.indice_cargas<len(self.tipos_cargas):
                ...
                print(self.qz)
                self.tipos_cargas_puntuales.append("Hay alguna carga puntual en algun apoyo?:")
            ...
            if self.respuestas_cargas_puntuales[0]=="s":
                if self.respuesta_tipo_de_problema=="portico":
                    for i in range(0, int(self.respuestas_tramos[0])):
                        self.tipos_cargas_puntuales.append(f"Ingrese componente cortante (respecto ejes tramo {i+1}) de la carga puntual del apoyo {i+1}:")
                        self.tipos_cargas_puntuales.append(f"Ingrese componente axial (respecto ejes tramo {i+1}) de la carga puntual del apoyo {i+1}:")
                    ...
                    for i in range(0, int(self.respuestas_tramos[0])+1):
                        self.tipos_cargas_puntuales.append(f"Ingrese componente cortante de la carga puntual del apoyo {i+1}:")
                        self.tipos_cargas_puntuales.append(f"Ingrese componente axial de la carga puntual del apoyo {i+1}:")
                self.indice_cargas_puntuales = self.indice_cargas_puntuales + 1
                self.pregunta_actual = self.tipos_cargas_puntuales[self.indice_cargas_puntuales]
            elif self.respuestas_cargas_puntuales[0]=="n":
            ...
            if self.indice_cargas_puntuales<len(self.tipos_cargas_puntuales):
                ...
                for i in range(1, int(self.respuestas_tramos[0]) +2):
                    ...
                    indice=indice+2
                #self.Qz.append(parse_expr(sympify(self.respuestas_cargas_puntuales[1::2]), local_dict=self.simbolos_conocidos))
                #self.Qz.extend([sympify(val) for val in self.respuestas_cargas_puntuales[1::2]])
                #self.Qx.extend([sympify(val) for val in self.respuestas_cargas_puntuales[2::2]])
                #self.Qz = [var for sublist in self.Qz for var in sublist]
                print(self.Qz)
                #self.Qx = [var for sublist in self.Qx for var in sublist]
                print(self.Qx)
    ...
    def calculo_simbolico(self):
        ...
        #for eq in self.ecuaciones_0:
         #   if not any(simplify(eq - otra) == 0 for otra in self.ecuaciones_0_filtradas):
          #      self.ecuaciones_0_filtradas.append(eq)
        #print(f"primera filtracion {self.ecuaciones_0_filtradas}")
        #self.ecuaciones = [expr.subs({var: 0 for var in self.ecuaciones_0_filtradas}) for expr in self.ecuaciones]
        #self.ecuaciones.append(self.ecuaciones_0_filtradas)
        #print(f"sustituciones en la principal {self.ecuaciones}")
        #for eq in self.ecuaciones:
         #   if not any(simplify(eq - otra) == 0 for otra in self.ecuaciones_filtradas):
          #      self.ecuaciones_filtradas.append(eq)
        ...
        i = 0
        self.solution_text.insert(tk.END,"\n      HAZ DE SOLUCIONES CON LAS SOLUCIONES PARTICULARES SUSTITUIDAS:\n")
        for j in range(0, int(self.respuestas_tramos[0])):
            self.solution_text.insert(tk.END,f"Para el tramo {j + 1}\n"
    ...
    def graficas(self):
        ...
        for i, expr in enumerate(expresiones):
            ...
            self.ax_graficas.plot(x_global, y_vals, label=f"Ecuación Tramo{i + 1}")
            x_offset=x_offset+float(self.L_val[self.L[i]])

        self.ax_graficas.set_xlabel("x")
        self.ax_graficas.set_ylabel("y")
